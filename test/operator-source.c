...
static void
set_solid_pattern (comac_t *cr, int x, int y)
{
    comac_set_source_rgb (cr, 1.0, 0, 0.0);
}

static void
set_translucent_pattern (comac_t *cr, int x, int y)
{
    comac_set_source_rgba (cr, 1, 0, 0, 0.5);
}

static void
set_gradient_pattern (comac_t *cr, int x, int y)
{
    comac_pattern_t *pattern;

    pattern = comac_pattern_create_linear (x, y, x + WIDTH, y + HEIGHT);
    comac_pattern_add_color_stop_rgba (pattern, 0.2, 1, 0, 0, 1);
    comac_pattern_add_color_stop_rgba (pattern, 0.8, 1, 0, 0, 0.0);
    comac_set_source (cr, pattern);
    comac_pattern_destroy (pattern);
}

static void
set_surface_pattern (comac_t *cr, int x, int y)
{
    ...
    source_surface = comac_surface_create_similar (comac_get_group_target (cr),
						   COMAC_CONTENT_COLOR_ALPHA,
						   width,
						   height);
    cr2 = comac_create (source_surface);
    comac_surface_destroy (source_surface);
    ...
    comac_arc (cr2, 0.5 * width, 0.5 * height, 0.5 * height, 0, 2 * M_PI);
    comac_fill (cr2);

    comac_set_source_surface (cr, comac_get_target (cr2), x, y);
    comac_destroy (cr2);
}
...
static void
draw_rects (comac_t *cr, int x, int y, double offset)
{
...
static void
draw_aligned_rects (comac_t *cr, int x, int y)
{
    draw_rects (cr, x, y, 0);
}

static void
draw_unaligned_rects (comac_t *cr, int x, int y)
{
    draw_rects (cr, x, y, 2.1);
}

static void (*const pattern_funcs[]) (comac_t *cr, int x, int y) = {
    set_solid_pattern,
...
COMAC_TEST (operator_source,
	    "Test of COMAC_OPERATOR_SOURCE",
	    "operator", /* keywords */
	    NULL,	/* requirements */
	    IMAGE_WIDTH,
	    IMAGE_HEIGHT,
