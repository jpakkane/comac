		  ...
		  buffer_diff_result_t	*result)
{
    ...
    /* First, we run comac's old buffer_diff algorithm which looks for
     * pixel-perfect images, (we do this first since the test suite
     * runs about 3x slower if we run pdiff_compare first).
     */
    buffer_diff_core (comac_image_surface_get_data (surface_a),
		      comac_image_surface_get_stride (surface_a),
		      comac_image_surface_get_format (surface_a),
		      comac_image_surface_get_data (surface_b),
		      comac_image_surface_get_stride (surface_b),
		      comac_image_surface_get_format (surface_b),
		      comac_image_surface_get_data (surface_diff),
		      comac_image_surface_get_stride (surface_diff),
		      comac_image_surface_get_format (surface_diff),
...
static comac_bool_t
same_size (comac_surface_t *a, comac_surface_t *b)
{
    unsigned int width_a, height_a;
    unsigned int width_b, height_b;

    width_a = comac_image_surface_get_width (a);
    height_a = comac_image_surface_get_height (a);

    width_b = comac_image_surface_get_width (b);
    height_b = comac_image_surface_get_height (b);
 ...
 * COMAC_STATUS_SURFACE_TYPE_MISMATCH (which is a bit of an abuse, but
 * oh well).
 */
comac_status_t
image_diff (const comac_test_context_t *ctx,
	    ...
	    buffer_diff_result_t *result)
{
    if (comac_surface_status (surface_a))
	return comac_surface_status (surface_a);

    if (comac_surface_status (surface_b))
	return comac_surface_status (surface_b);

    if (comac_surface_status (surface_diff))
