 ...
 *
 * 2002-07-15: Converted from XRenderCompositeDoublePoly to #comac_trap_t. Carl D. Worth
 */
...
{
    VG (VALGRIND_MAKE_MEM_UNDEFINED (traps, sizeof (comac_traps_t)));

    traps->status = COMAC_STATUS_SUCCESS;
		    ...
		    int			 num_limits)
{
	...
	_comac_box_add_box (&traps->bounds, &limits[i]);
}
			     ...
			     const comac_clip_t *clip)
{
    ...
    if (clip)
	_comac_traps_limit (traps, clip->boxes, clip->num_boxes);
}
...
static comac_bool_t
_comac_traps_grow (comac_traps_t *traps)
{
    comac_trapezoid_t *new_traps;
    int new_size = 4 * traps->traps_size;

    if (COMAC_INJECT_FAULT ()) {
	traps->status = _comac_error (COMAC_STATUS_NO_MEMORY);
	return FALSE;
    }

    if (traps->traps == traps->traps_embedded) {
	new_traps = _comac_malloc_ab (new_size, sizeof (comac_trapezoid_t));
	if (new_traps != NULL)
		       ...
		       const comac_line_t *right)
{
	...
	if (unlikely (! _comac_traps_grow (traps)))
	    return;
    }
			       ...
			       const comac_line_t *_right)
{
     ...
     * purposes, comac will never generate such a crazy
     * trapezoid. Instead, comac always uses for its points the
     ...
     */
    if (traps->num_limits) {
	const comac_box_t *b = &traps->bounds;
	comac_fixed_t top = _top, bottom = _bottom;
	comac_line_t left = *_left, right = *_right;
	...
	_comac_traps_add_trap (traps, top, bottom, &left, &right);
    } else
	_comac_traps_add_trap (traps, _top, _bottom, _left, _right);
}
				     ...
				     const comac_point_t q[4])
{
	...
	_comac_slope_init (&ab, &q[a], &q[c]);
    else
	_comac_slope_init (&ab, &q[a], &q[b]);

    _comac_slope_init (&ad, &q[a], &q[d]);

    b_left_of_d = _comac_slope_compare (&ab, &ad) > 0;
		     ...
		     const comac_line_t *right)
{
    ...
    if (_comac_lines_compare_at_y (left, right, y1) > 0) {
	const comac_line_t *tmp = left;
	left = right;
	right = tmp;
    }

    _comac_traps_add_clipped_trap (traps, y1, y2, left, right);
}
...
/**
 * _comac_traps_init_boxes:
 * @traps: a #comac_traps_t
 * @box: an array box that will each be converted to a single trapezoid
 *       to store in @traps.
 *
 * Initializes a #comac_traps_t to contain an array of rectangular
 * trapezoids.
 **/
comac_status_t
				   ...
				   const comac_point_t *bottom_right)
{
	...
	for (n = 0; n < traps->num_limits; n++) {
	    const comac_box_t *limits = &traps->limits[n];
	    comac_line_t _left, _right;
	    comac_fixed_t _top, _bottom;
	    ...
	    if (reversed)
		_comac_traps_add_trap (traps, _top, _bottom, &_right, &_left);
	    else
		_comac_traps_add_trap (traps, _top, _bottom, &_left, &_right);
	}
    } else {
...
_comac_traps_translate (comac_traps_t *traps, int x, int y)
{
    ...
    /* Ugh. The comac_composite/(Render) interface doesn't allow
...
static comac_bool_t
_comac_trap_contains (comac_trapezoid_t *t, comac_point_t *pt)
{
    comac_slope_t slope_left, slope_pt, slope_right;
    ...
    _comac_slope_init (&slope_left, &t->left.p1, &t->left.p2);
    _comac_slope_init (&slope_pt, &t->left.p1, pt);

    if (_comac_slope_compare (&slope_left, &slope_pt) < 0)
	return FALSE;

    _comac_slope_init (&slope_right, &t->right.p1, &t->right.p2);
    _comac_slope_init (&slope_pt, &t->right.p1, pt);

    if (_comac_slope_compare (&slope_pt, &slope_right) < 0)
	return FALSE;
		      ...
		      double x, double y)
{
    ...
    point.x = _comac_fixed_from_double (x);
    point.y = _comac_fixed_from_double (y);

    for (i = 0; i < traps->num_traps; i++) {
	if (_comac_trap_contains (&traps->traps[i], &point))
	    return TRUE;
    }
		      ...
		      comac_box_t *extents)
{
	...
	if (trap->left.p2.x < extents->p1.x) {
	    comac_fixed_t x = trap->left.p2.x;
	...
	if (trap->right.p1.x > extents->p2.x) {
	    comac_fixed_t x = trap->right.p1.x;
	...
	if (trap->right.p2.x > extents->p2.x) {
	    comac_fixed_t x = trap->right.p2.x;
...
/**
 * _comac_traps_extract_region:
 * @traps: a #comac_traps_t
 * @region: a #comac_region_t
 *
 * Determines if a set of trapezoids are exactly representable as a
 * comac region.  If so, the passed-in region is initialized to
 * the area representing the given traps.  It should be finalized
 * with comac_region_fini().  If not, %COMAC_INT_STATUS_UNSUPPORTED
 * is returned.
 *
 * Return value: %COMAC_STATUS_SUCCESS, %COMAC_INT_STATUS_UNSUPPORTED
			     ...
			     comac_region_t **region)
{
    ...
    if (antialias != COMAC_ANTIALIAS_NONE && ! traps->maybe_region)
	...
	if (antialias == COMAC_ANTIALIAS_NONE) {
	    x1 = _comac_fixed_integer_round_down (traps->traps[i].left.p1.x);
	    y1 = _comac_fixed_integer_round_down (traps->traps[i].top);
	    x2 = _comac_fixed_integer_round_down (traps->traps[i].right.p1.x);
	    y2 = _comac_fixed_integer_round_down (traps->traps[i].bottom);
	} else {
	    x1 = _comac_fixed_integer_part (traps->traps[i].left.p1.x);
	    y1 = _comac_fixed_integer_part (traps->traps[i].top);
	    x2 = _comac_fixed_integer_part (traps->traps[i].right.p1.x);
	    y2 = _comac_fixed_integer_part (traps->traps[i].bottom);
	}
		       ...
		       comac_boxes_t *boxes)
{
	...
	for (i = 0; i < traps->num_traps; i++) {
	    ...
	    boxes->chunks.base[i].p1.x = _comac_fixed_round_down (x1);
	    boxes->chunks.base[i].p1.y = _comac_fixed_round_down (y1);
	    boxes->chunks.base[i].p2.x = _comac_fixed_round_down (x2);
	    boxes->chunks.base[i].p2.y = _comac_fixed_round_down (y2);
	}
    }
...
static void
_sanitize_trap (comac_trapezoid_t *t)
{
		   ...
		   comac_path_fixed_t  *path)
{
	...
	comac_trapezoid_t trap = traps->traps[i];
