...
#define _comac_atomic_int_inc(x)                                               \
    ((void) __atomic_fetch_add (x, 1, __ATOMIC_SEQ_CST))
#define _comac_atomic_int_dec(x)                                               \
    ((void) __atomic_fetch_sub (x, 1, __ATOMIC_SEQ_CST))
#define _comac_atomic_int_dec_and_test(x)                                      \
    (__atomic_fetch_sub (x, 1, __ATOMIC_SEQ_CST) == 1)
				...
				comac_atomic_int_t newv)
{
    ...
    return __atomic_compare_exchange_n (x,
					&expected,
					newv,
					0,
					__ATOMIC_SEQ_CST,
...
{
    comac_atomic_int_t expected = oldv;
    (void) __atomic_compare_exchange_n (x,
					&expected,
					newv,
					0,
					__ATOMIC_SEQ_CST,
...
#define _comac_atomic_int_cmpxchg(x, oldv, newv)                               \
    __sync_bool_compare_and_swap (x, oldv, newv)
#define _comac_atomic_int_cmpxchg_return_old(x, oldv, newv)                    \
    __sync_val_compare_and_swap (x, oldv, newv)
...
#define _comac_atomic_ptr_cmpxchg(x, oldv, newv)                               \
    __sync_bool_compare_and_swap ((comac_atomic_intptr_t *) x,                 \
				  (comac_atomic_intptr_t) oldv,                \
				  (comac_atomic_intptr_t) newv)

#define _comac_atomic_ptr_cmpxchg_return_old(x, oldv, newv)                    \
    _comac_atomic_intptr_to_voidptr (                                          \
	__sync_val_compare_and_swap ((comac_atomic_intptr_t *) x,              \
				     (comac_atomic_intptr_t) oldv,             \
				     (comac_atomic_intptr_t) newv))
...
#define _comac_atomic_int_cmpxchg(x, oldv, newv)                               \
    AO_compare_and_swap_full (x, oldv, newv)
...
#define _comac_atomic_ptr_get(x)                                               \
    _comac_atomic_intptr_to_voidptr (AO_load_full (x))
#define _comac_atomic_ptr_cmpxchg(x, oldv, newv)                               \
    _comac_atomic_int_cmpxchg ((comac_atomic_intptr_t *) (x),                  \
			       (comac_atomic_intptr_t) oldv,                   \
			       (comac_atomic_intptr_t) newv)
...
#define _comac_atomic_int_cmpxchg(x, oldv, newv)                               \
    OSAtomicCompareAndSwap32Barrier (oldv, newv, x)

#if SIZEOF_VOID_P == 4
typedef int32_t comac_atomic_intptr_t;
#define _comac_atomic_ptr_cmpxchg(x, oldv, newv)                               \
    OSAtomicCompareAndSwap32Barrier ((comac_atomic_intptr_t) oldv,             \
				     (comac_atomic_intptr_t) newv,             \
				     (comac_atomic_intptr_t *) x)

#elif SIZEOF_VOID_P == 8
typedef int64_t comac_atomic_intptr_t;
#define _comac_atomic_ptr_cmpxchg(x, oldv, newv)                               \
    OSAtomicCompareAndSwap64Barrier ((comac_atomic_intptr_t) oldv,             \
				     (comac_atomic_intptr_t) newv,             \
				     (comac_atomic_intptr_t *) x)
...
_comac_atomic_int_cmpxchg (comac_atomic_int_t *x,
			   ...
			   comac_atomic_int_t newv)
{
    return InterlockedCompareExchange ((unsigned int *) x,
				       (unsigned int) newv,
				       (unsigned int) oldv) == oldv;
}
...
_comac_atomic_int_set_relaxed (comac_atomic_int_t *x, comac_atomic_int_t val);
#define _comac_atomic_ptr_get(x)                                               \
    (void *) _comac_atomic_int_get ((comac_atomic_int_t *) x)
#else
...
static comac_always_inline comac_atomic_int_t
_comac_atomic_int_cmpxchg_return_old_fallback (comac_atomic_int_t *x,
					       comac_atomic_int_t oldv,
					       comac_atomic_int_t newv)
{
...
#define _comac_atomic_int_cmpxchg_return_old(x, oldv, newv)                    \
    _comac_atomic_int_cmpxchg_return_old_fallback (x, oldv, newv)
#endif

#ifndef _comac_atomic_ptr_cmpxchg_return_old
#define _comac_atomic_ptr_cmpxchg_return_old(x, oldv, newv)                    \
    _comac_atomic_ptr_cmpxchg_return_old_fallback (x, oldv, newv)
#endif

#ifndef _comac_atomic_int_cmpxchg
#define _comac_atomic_int_cmpxchg(x, oldv, newv)                               \
    (_comac_atomic_int_cmpxchg_return_old (x, oldv, newv) == oldv)
#endif

#ifndef _comac_atomic_ptr_cmpxchg
#define _comac_atomic_ptr_cmpxchg(x, oldv, newv)                               \
    (_comac_atomic_ptr_cmpxchg_return_old (x, oldv, newv) == oldv)
#endif
...
_comac_atomic_init_once_leave (comac_atomic_once_t *once)
{
	...
	assert (0 && "incorrect use of _comac_atomic_init_once API (once != "
		     "COMAC_ATOMIC_ONCE_INITIALIZING)");
}
