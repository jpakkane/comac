...
{
    TRACE ((stderr, "%s\n", __FUNCTION__));
    return _pixman_image_for_color (COMAC_COLOR_TRANSPARENT);
}
...
{
    TRACE ((stderr, "%s\n", __FUNCTION__));
    return _pixman_image_for_color (COMAC_COLOR_BLACK);
}
...
{
    TRACE ((stderr, "%s\n", __FUNCTION__));
    return _pixman_image_for_color (COMAC_COLOR_WHITE);
}
...
_pixman_image_for_color (const comac_color_t *comac_color)
{
    ...
    if (COMAC_COLOR_IS_CLEAR (comac_color))
    ...
    COMAC_MUTEX_LOCK (_comac_image_solid_cache_mutex);
    for (i = 0; i < n_cached; i++) {
	if (_comac_color_equal (&cache[i].color, comac_color)) {
    ...
    cache[i].color = *comac_color;

UNLOCK:
    COMAC_MUTEX_UNLOCK (_comac_image_solid_cache_mutex);
#endif
    return image;
}
			    ...
			    int *iy)
{
    ...
    p2.x = _comac_fixed_16_16_from_double (extremes[1].center.x);
...
_acquire_source_cleanup (pixman_image_t *pixman_image, void *closure)
{
    ...
    _comac_surface_release_source_image (data->surface,
					 data->image,
					 data->image_extra);
    free (data);
}
...
static pixman_image_t *
_pixel_to_solid (comac_image_surface_t *image, int x, int y)
{
    uint32_t pixel;
    float *rgba;
    pixman_color_t color;
			      ...
			      int *iy)
{
	...
	case COMAC_FILTER_GAUSSIAN:
	    /* XXX: The GAUSSIAN value has no implementation in comac
...
static pixman_image_t *
_pixman_image_for_recording (comac_image_surface_t *dst,
			     ...
			     int *iy)
{
    ...
    comac_rectangle_int_t src_limit;
    pixman_image_t *pixman_image;
    comac_status_t status;
    comac_extend_t extend;
    comac_matrix_t *m, matrix;
    ...
    *ix = *iy = 0;
    ...
    source = _comac_pattern_get_source (pattern, &limit);
    src_limit = limit;

    extend = pattern->base.extend;
    if (_comac_rectangle_contains_rectangle (&limit, sample))
	extend = COMAC_EXTEND_NONE;

    if (extend == COMAC_EXTEND_NONE) {
	if (! _comac_rectangle_intersect (&limit, sample))
	    return _pixman_transparent_image ();
    }

    if (! _comac_matrix_is_identity (&pattern->base.matrix)) {
	double x1, y1, x2, y2;
    ...
    if (proxy != NULL) {
	clone = comac_surface_reference (get_proxy (proxy));
	goto done;
    }
	...
	matrix = pattern->base.matrix;
	if (tx | ty)
	    comac_matrix_translate (&matrix, tx, ty);
	m = &matrix;
    } else {
    ...
    status = _comac_recording_surface_replay_with_clip (source, m, clone, NULL);
...
static pixman_image_t *
_pixman_image_for_surface (comac_image_surface_t *dst,
			   const comac_surface_pattern_t *pattern,
			   comac_bool_t is_mask,
			   const comac_rectangle_int_t *extents,
			   const comac_rectangle_int_t *sample,
			   ...
			   int *iy)
{
    ...
    *ix = *iy = 0;
	...
	if (_comac_surface_is_snapshot (&source->base)) {
	    defer_free = _comac_surface_snapshot_get_target (&source->base);
	    source = (comac_image_surface_t *) defer_free;
	}
	...
	} else if (type == COMAC_SURFACE_TYPE_SUBSURFACE) {
	    comac_surface_subsurface_t *sub;
	    comac_bool_t is_contained = FALSE;
...
static pixman_image_t *
_pixman_image_for_raster (comac_image_surface_t *dst,
			  ...
			  int *iy)
{
    ...
    *ix = *iy = 0;
    ...
    status = _comac_surface_acquire_source_image (surface, &image, &extra);
    if (unlikely (status)) {
	_comac_raster_source_pattern_release (&pattern->base, surface);
	return NULL;
    }
    ...
    pixman_image = pixman_image_create_bits (image->pixman_format,
					     ...
					     image->stride);
    if (unlikely (pixman_image == NULL)) {
	_comac_surface_release_source_image (surface, image, extra);
	_comac_raster_source_pattern_release (&pattern->base, surface);
	return NULL;
    }
...
static comac_status_t
_comac_image_source_finish (void *abstract_surface)
{
    comac_image_source_t *source = abstract_surface;

    pixman_image_unref (source->pixman_image);
    return COMAC_STATUS_SUCCESS;
}

const comac_surface_backend_t _comac_image_source_backend = {
    COMAC_SURFACE_TYPE_IMAGE,
    _comac_image_source_finish,
					...
					int *src_y)
{
    ...
    _comac_surface_init (&source->base,
