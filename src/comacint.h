...
#if COMAC_HAS_PDF_SURFACE || COMAC_HAS_PS_SURFACE ||                           \
    COMAC_HAS_SCRIPT_SURFACE || COMAC_HAS_XML_SURFACE
#define COMAC_HAS_DEFLATE_STREAM 1
#endif

#if COMAC_HAS_PS_SURFACE || COMAC_HAS_PDF_SURFACE || COMAC_HAS_SVG_SURFACE ||  \
    COMAC_HAS_WIN32_SURFACE
#define COMAC_HAS_FONT_SUBSET 1
#endif

#if COMAC_HAS_PS_SURFACE || COMAC_HAS_PDF_SURFACE || COMAC_HAS_FONT_SUBSET
...
#define COMPILE_TIME_ASSERT1(condition, line)                                  \
    typedef int                                                                \
	compile_time_assertion_at_line_##line##_failed[(condition) ? 1 : -1]
#define COMPILE_TIME_ASSERT0(condition, line)                                  \
    COMPILE_TIME_ASSERT1 (condition, line)
#define COMPILE_TIME_ASSERT(condition)                                         \
    COMPILE_TIME_ASSERT0 (condition, __LINE__)

#define COMAC_ALPHA_IS_CLEAR(alpha)                                            \
    ((alpha) <= ((double) 0x00ff / (double) 0xffff))
#define COMAC_ALPHA_SHORT_IS_CLEAR(alpha) ((alpha) <= 0x00ff)

#define COMAC_ALPHA_IS_OPAQUE(alpha)                                           \
    ((alpha) >= ((double) 0xff00 / (double) 0xffff))
#define COMAC_ALPHA_SHORT_IS_OPAQUE(alpha) ((alpha) >= 0xff00)
#define COMAC_ALPHA_IS_ZERO(alpha) ((alpha) <= 0.0)

#define COMAC_COLOR_IS_CLEAR(color)                                            \
    COMAC_ALPHA_SHORT_IS_CLEAR ((color)->alpha_short)
#define COMAC_COLOR_IS_OPAQUE(color)                                           \
    COMAC_ALPHA_SHORT_IS_OPAQUE ((color)->alpha_short)
...
get_unaligned_be16 (const unsigned char *p)
{
    return p[0] << 8 | p[1];
}

static inline uint32_t
get_unaligned_be32 (const unsigned char *p)
{
    return (uint32_t) p[0] << 24 | p[1] << 16 | p[2] << 8 | p[3];
}

static inline void
put_unaligned_be16 (uint16_t v, unsigned char *p)
{
    p[0] = (v >> 8) & 0xff;
    p[1] = v & 0xff;
}
...
static inline void
put_unaligned_be32 (uint32_t v, unsigned char *p)
{
    p[0] = (v >> 24) & 0xff;
    p[1] = (v >> 16) & 0xff;
    p[2] = (v >> 8) & 0xff;
...
comac_private uintptr_t
_comac_hash_bytes (uintptr_t hash, const void *bytes, unsigned int length);

/* We use bits 24-27 to store phases for subpixel positions */
#define _comac_scaled_glyph_index(g)                                           \
    ((unsigned long) ((g)->hash_entry.hash & 0xffffff))
...
struct _comac_scaled_font_backend {
     ...
     */
    comac_warn comac_int_status_t (*scaled_glyph_init) (
	void *scaled_font,
     ...
     */
    comac_warn comac_int_status_t (*text_to_glyphs) (
	void *scaled_font,
	double x,
	double y,
	const char *utf8,
     ...
     */
    unsigned long (*ucs4_to_index) (void *scaled_font, uint32_t ucs4);
     ...
     */
    comac_warn
	comac_int_status_t (*load_truetype_table) (void *scaled_font,
						   unsigned long tag,
						   long offset,
     ...
     * for the glyph */
    comac_warn comac_int_status_t (*index_to_ucs4) (void *scaled_font,
						    unsigned long index,
						    uint32_t *ucs4);
     ...
     */
    comac_warn comac_int_status_t (*is_synthetic) (void *scaled_font,
     ...
     *  to the glyph_index
     */
    comac_warn comac_int_status_t (*index_to_glyph_name) (
	void *scaled_font,
	char **glyph_names,
	int num_glyph_names,
     ...
     */
    comac_warn comac_int_status_t (*load_type1_data) (void *scaled_font,
						      long offset,
						      unsigned char *buffer,
						      unsigned long *length);
     ...
     */
    comac_bool_t (*has_color_glyphs) (void *scaled_font);
};
...
struct _comac_font_face_backend {
    ...
    comac_warn
	comac_status_t (*create_for_toy) (comac_toy_font_face_t *toy_face,
					  comac_font_face_t **font_face);

    /* The destroy() function is allowed to resurrect the font face
     * by re-referencing. This is needed for the FreeType backend.
     */
    comac_bool_t (*destroy) (void *font_face);

    comac_warn comac_status_t (*scaled_font_create) (
	void *font_face,
    ...
    comac_font_face_t *(*get_implementation) (
	void *font_face,
	const comac_matrix_t *font_matrix,
	const comac_matrix_t *ctm,
	const comac_font_options_t *options);
...
extern const comac_private struct _comac_font_face_backend
    _comac_user_font_face_backend;

/* concrete font backends */
#if COMAC_HAS_FT_FONT

extern const comac_private struct _comac_font_face_backend
    _comac_ft_font_face_backend;
...
extern const comac_private struct _comac_font_face_backend
    _comac_win32_font_face_backend;
...
extern const comac_private struct _comac_font_face_backend
    _comac_dwrite_font_face_backend;
...
extern const comac_private struct _comac_font_face_backend
    _comac_quartz_font_face_backend;
...
extern const comac_private comac_font_face_t
    _comac_font_face_nil_file_not_found;
...
typedef comac_status_t (comac_path_fixed_move_to_func_t) (
    void *closure, const comac_point_t *point);

typedef comac_status_t (comac_path_fixed_line_to_func_t) (
    void *closure, const comac_point_t *point);

typedef comac_status_t (comac_path_fixed_curve_to_func_t) (
    void *closure,
    const comac_point_t *p0,
    const comac_point_t *p1,
    const comac_point_t *p2);

typedef comac_status_t (comac_path_fixed_close_path_func_t) (void *closure);
...
comac_private comac_bool_t
_comac_path_bounder_extents (const comac_path_fixed_t *path, comac_box_t *box);
...
_comac_path_fixed_fill_rectilinear_to_region (
    const comac_path_fixed_t *path,
    comac_fill_rule_t fill_rule,
...
_comac_path_fixed_stroke_to_shaper (
    ...
    comac_status_t (*add_triangle_fan) (void *closure,
					...
					int npoints),
    comac_status_t (*add_quad) (void *closure, const comac_point_t quad[4]),
    void *closure);
 ...
 */
#define COMAC_FORMAT_VALID(format)                                             \
    ((format) >= COMAC_FORMAT_ARGB32 && (format) <= COMAC_FORMAT_RGBA128F)

/* pixman-required stride alignment in bytes. */
#define COMAC_STRIDE_ALIGNMENT (sizeof (uint32_t))
#define COMAC_STRIDE_FOR_WIDTH_BPP(w, bpp)                                     \
    ((((bpp) * (w) + 7) / 8 + COMAC_STRIDE_ALIGNMENT - 1) &                    \
     -COMAC_STRIDE_ALIGNMENT)

#define COMAC_CONTENT_VALID(content)                                           \
    ((content) && (((content) & ~(COMAC_CONTENT_COLOR | COMAC_CONTENT_ALPHA |  \
				  COMAC_CONTENT_COLOR_ALPHA)) == 0))
...
comac_private comac_status_t
_comac_polygon_reduce (comac_polygon_t *polygon, comac_fill_rule_t fill_rule);

comac_private comac_status_t
_comac_polygon_intersect (comac_polygon_t *a,
			  int winding_a,
			  comac_polygon_t *b,
			  int winding_b);
...
{
    return (matrix->xx == 1.0 && matrix->yx == 0.0 && matrix->xy == 0.0 &&
	    matrix->yy == 1.0 && matrix->x0 == 0.0 && matrix->y0 == 0.0);
}
...
comac_private comac_status_t
_comac_bentley_ottmann_tessellate_rectilinear_polygon (
    comac_traps_t *traps,
...
#if _WIN32 || COMAC_HAS_WIN32_FONT || COMAC_HAS_QUARTZ_FONT ||                 \
    COMAC_HAS_PDF_OPERATORS
