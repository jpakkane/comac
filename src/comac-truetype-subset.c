...
typedef struct _comac_truetype_font comac_truetype_font_t;
			     ...
			     comac_truetype_font_t **font_return)
{
     ...
     *
     *  if (comac_font_options_get_vertical_layout (scaled_font_subset->scaled_font))
     *   return COMAC_INT_STATUS_UNSUPPORTED;
     */
    ...
    font = _comac_malloc (sizeof (comac_truetype_font_t));
    ...
    status = _comac_truetype_read_font_name (scaled_font_subset->scaled_font,
					     &font->base.ps_name,
					     &font->base.font_name);
    ...
    _comac_array_init (&font->string_offsets, sizeof (unsigned long));
    status = _comac_array_grow_by (&font->string_offsets, 10);
    if (unlikely (status))
	goto fail5;

    font->status = COMAC_STATUS_SUCCESS;
...
static void
comac_truetype_font_destroy (comac_truetype_font_t *font)
{
    _comac_array_fini (&font->string_offsets);
    free (font->widths);
    free (font->base.ps_name);
					   ...
					   unsigned char **buffer)
{
    ...
    status = _comac_array_allocate (&font->output, length, (void **) buffer);
    if (unlikely (status))
			   ...
			   size_t length)
{
    ...
    status = _comac_array_append_multiple (&font->output, data, length);
    if (unlikely (status))
	status = _comac_truetype_font_set_error (font, status);
}
...
comac_truetype_font_write_be16 (comac_truetype_font_t *font, uint16_t value)
{
    ...
    comac_truetype_font_write (font, &be16_value, sizeof be16_value);
}
...
comac_truetype_font_write_be32 (comac_truetype_font_t *font, uint32_t value)
{
    ...
    comac_truetype_font_write (font, &be32_value, sizeof be32_value);
}
				      ...
				      unsigned long tag)
{
    ...
    comac_truetype_font_write_be32 (font, 12); /* Offset to start of table */
				      ...
				      unsigned long tag)
{
    ...
    start_offset = _comac_array_num_elements (&font->output);
				      ...
				      unsigned long tag)
{
    ...
    font->checksum_index = _comac_array_num_elements (&font->output) + 8;
    status = comac_truetype_font_allocate_write_buffer (font, size, &buffer);
    if (unlikely (status))
	return _comac_truetype_font_set_error (font, status);
				      ...
				      unsigned long tag)
{
	...
	for (i = 0; i < font->num_glyphs + 1; i++)
	    comac_truetype_font_write_be32 (font, font->glyphs[i].location);
    }
...
static comac_status_t
comac_truetype_font_write_offset_table (comac_truetype_font_t *font)
{
    comac_status_t status;
    ...
    comac_truetype_font_write_be32 (font, SFNT_VERSION);
    comac_truetype_font_write_be16 (font, font->num_tables);
    comac_truetype_font_write_be16 (font, search_range);
    comac_truetype_font_write_be16 (font, entry_selector);
    comac_truetype_font_write_be16 (font, range_shift);
			      ...
			      unsigned long *num_strings)
{
    ...
    status = comac_truetype_font_write_offset_table (font);
    if (unlikely (status))
	goto FAIL;

    status = comac_truetype_font_align_output (font, &start);
    if (unlikely (status))
	goto FAIL;
	...
	end = _comac_array_num_elements (&font->output);
	status = comac_truetype_font_align_output (font, &next);
	if (unlikely (status))
	    goto FAIL;
    ...
    *data = _comac_array_index (&font->output, 0);
    *length = _comac_array_num_elements (&font->output);
    *num_strings = _comac_array_num_elements (&font->string_offsets);
    if (*num_strings != 0)
	*string_offsets = _comac_array_index (&font->string_offsets, 0);
    else
	*string_offsets = NULL;
...
/* comac_truetype_font_create_truetype_table_list() builds the list of
 * truetype tables to be embedded in the subsetted font. Each call to
 * comac_truetype_font_add_truetype_table() adds a table, the callback
 ...
 * using comac_truetype_font_add_truetype_table() specifies the order
 ...
 * comac_truetype_font_add_truetype_table() specifies the position of
 * the table in the table directory.
 */
static void
comac_truetype_font_create_truetype_table_list (comac_truetype_font_t *font)
{
    comac_bool_t has_cvt = FALSE;
    comac_bool_t has_fpgm = FALSE;
    comac_bool_t has_prep = FALSE;
    unsigned long size;
    int pos;
				     ...
				     comac_bool_t is_pdf)
{
    ...
    status = _comac_truetype_font_create (font_subset, is_pdf, &font);
    if (unlikely (status))
	return status;
	...
	truetype_subset->string_offsets = _comac_malloc (offsets_length);
...
void
_comac_truetype_subset_fini (comac_truetype_subset_t *subset)
{
    free (subset->ps_name);
			       ...
			       uint32_t *ucs4)
{
    ...
    cmap = _comac_malloc_ab_plus_c (num_tables, sizeof (tt_cmap_index_t), 4);
