...
/**
 * _comac_cache_init:
 * @cache: the #comac_cache_t to initialise
 * @keys_equal: a function to return %TRUE if two keys are equal
 * @entry_destroy: destroy notifier for cache entries
 * @max_size: the maximum size for this cache
 * Returns: the newly created #comac_cache_t
 *
 ...
 * of #comac_cache_entry_t. A cache entry must be able to hold hash
 * code, a size, and the key/value pair being stored in the
 * cache. Sometimes only the key will be necessary, (as in
 * _comac_cache_lookup()), and in these cases the value portion of the
 ...
 * adding an entry with _comac_cache_insert() if the total size of
 ...
 * _comac_cache_freeze() and _comac_cache_thaw() calls can be
		   ...
		   unsigned long		 max_size)
{
    ...
    if (predicate == NULL)
	predicate = _comac_cache_entry_is_non_zero;
...
static void
_comac_cache_pluck (void *entry, void *closure)
{
    _comac_cache_remove (closure, entry);
}
 ...
 * function, (as passed to _comac_cache_init()), will be called for
 ...
 * long as the cache is "frozen", calls to _comac_cache_insert() will
 * add new entries to the cache regardless of how large the cache
 * grows. See _comac_cache_thaw().
 *
 * Note: Multiple calls to _comac_cache_freeze() will stack, in that
 * the cache will remain "frozen" until a corresponding number of
 * calls are made to _comac_cache_thaw().
 **/
void
_comac_cache_freeze (comac_cache_t *cache)
{
 ...
 * precious
 *
 * Cancels the effects of _comac_cache_freeze().
 *
 * When a number of calls to _comac_cache_thaw() is made corresponding
 * to the number of calls to _comac_cache_freeze() the cache will no
 ...
 * automatic ejection of entries on _comac_cache_insert() will resume.
 **/
void
_comac_cache_thaw (comac_cache_t *cache)
{
	...
	_comac_cache_shrink_to_accommodate (cache, 0);
}
 ...
 **/
static comac_bool_t
_comac_cache_remove_random (comac_cache_t *cache)
{
    ...
    _comac_cache_remove (cache, entry);

    return TRUE;
}

/**
 * _comac_cache_shrink_to_accommodate:
 * @cache: a cache
		     ...
		     comac_cache_entry_t *entry)
{
	...
	_comac_cache_shrink_to_accommodate (cache, entry->size);

    status = _comac_hash_table_insert (cache->hash_table,
