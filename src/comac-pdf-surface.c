...
#include "comac-image-info-private.h"
 ...
 *  - Object stream
 *
 * Calling _comac_output_stream_printf (surface->output, ...) will
 * write to the currently open stream.
 *
 * PDF Stream:
 *   A PDF Stream may be opened and closed with the following functions:
 *     _comac_pdf_surface_open stream ()
 *     _comac_pdf_surface_close_stream ()
 *
 ...
 *     _comac_pdf_surface_open_content_stream ()
 *     _comac_pdf_surface_close_content_stream ()
 *
 ...
 *     _comac_pdf_surface_open_group ()
 *     _comac_pdf_surface_close_group ()
 *
 ...
 *     _comac_pdf_surface_open_object_stream ()
 *     _comac_pdf_surface_close_object_stream ()
 *
 ...
 *  _comac_pdf_surface_object_begin()/_comac_pdf_surface_object_end().
 */
 ...
 *
 * The PDF surface is used to render comac graphics to Adobe
 * PDF files and is a multi-page vector surface backend.
 *
 * The following mime types are supported: %COMAC_MIME_TYPE_JPEG,
 * %COMAC_MIME_TYPE_JP2, %COMAC_MIME_TYPE_UNIQUE_ID,
 * %COMAC_MIME_TYPE_JBIG2, %COMAC_MIME_TYPE_JBIG2_GLOBAL,
 ...
 * %COMAC_MIME_TYPE_JBIG2.  Any global segments in the JBIG2 data
 * (segments with page association field set to 0) must be in
 * %COMAC_MIME_TYPE_JBIG2_GLOBAL. The global data may be shared by
 * multiple images. All images sharing the same global data must set
 * %COMAC_MIME_TYPE_JBIG2_GLOBAL_ID to a unique identifier. At least
 * one of the images must provide the global data using
 * %COMAC_MIME_TYPE_JBIG2_GLOBAL. The global data will only be
 * embedded once and shared by all JBIG2 images with the same
 * %COMAC_MIME_TYPE_JBIG2_GLOBAL_ID.
 *
 * # CCITT Fax Images # {#ccitt}
 * The %COMAC_MIME_TYPE_CCITT_FAX mime data requires a number of decoding
 * parameters These parameters are specified using %COMAC_MIME_TYPE_CCITT_FAX_PARAMS.
 *
 * %COMAC_MIME_TYPE_CCITT_FAX_PARAMS mime data must contain a string of the form
 ...
 *
 * An example %COMAC_MIME_TYPE_CCITT_FAX_PARAMS string is:
 *
 * <programlisting>
...
typedef struct _comac_xref_stream_object {
    comac_pdf_resource_t resource;
    long long offset;
} comac_xref_stream_object_t;

typedef struct _comac_pdf_font {
    unsigned int font_id;
    unsigned int subset_id;
    comac_pdf_resource_t subset_resource;
} comac_pdf_font_t;
...
static void
_comac_pdf_surface_clear (comac_pdf_surface_t *surface);
...
static comac_int_status_t
_comac_pdf_surface_write_pages (comac_pdf_surface_t *surface);

static comac_int_status_t
_comac_pdf_surface_write_catalog (comac_pdf_surface_t *surface,
				  comac_pdf_resource_t catalog);
...
_comac_pdf_surface_new_object (comac_pdf_surface_t *surface)
{
     ...
     * _comac_pdf_surface_object_begin() and Object Streams are
     * enabled it will be changed to Compressed. */
    object.type = PDF_OBJECT_UNCOMPRESSED;
    object.u.offset = _comac_output_stream_get_position (surface->output);

    status = _comac_array_append (&surface->objects, &object);
    if (unlikely (status)) {
	resource.id = 0;
	return resource;
    }
    ...
    comac_antialias_t antialias)
{
    ...
    if (path == NULL) {
	_comac_output_stream_printf (surface->output, "Q q\n");
    ...
    double height)
{
    ...
    surface = _comac_malloc (sizeof (comac_pdf_surface_t));
    ...
    surface->width = width;
    surface->height = height;
    comac_matrix_init (&surface->comac_to_pdf, 1, 0, 0, 1, 0, 0);
    ...
    _comac_pdf_group_resources_init (&surface->resources);

    surface->font_subsets = _comac_scaled_font_subsets_create_composite ();
...
BAIL1:
    _comac_hash_table_destroy (surface->all_surfaces);
BAIL0:
    _comac_array_fini (&surface->objects);
    free (surface);

    /* destroy stream on behalf of caller */
    status_ignored = _comac_output_stream_destroy (output);
 ...
 * @write_func: a #comac_write_func_t to accept the output data, may be %NULL
...
static comac_bool_t
_comac_surface_is_pdf (comac_surface_t *surface)
{
    return surface->backend == &comac_pdf_surface_backend;
}
		      ...
		      comac_pdf_surface_t **pdf_surface)
{
    ...
    *pdf_surface = (comac_pdf_surface_t *) target;
    return TRUE;
}

/**
 * comac_pdf_surface_restrict_to_version:
 * @surface: a PDF #comac_surface_t
 * @version: PDF version
 *
 * Restricts the generated PDF file to @version. See comac_pdf_get_versions()
...
comac_pdf_get_versions (comac_pdf_version_t const **versions, int *num_versions)
{
	...
	*num_versions = COMAC_PDF_VERSION_LAST;
}

/**
 * comac_pdf_version_to_string:
 * @version: a version id
 *
 * Get the string representation of the given @version id. This function
 * will return %NULL if @version isn't valid. See comac_pdf_get_versions()
...
comac_pdf_version_to_string (comac_pdf_version_t version)
{
    ...
    return _comac_pdf_version_strings[version];
}
 ...
 * comac_show_page() or comac_copy_page().
 *
 * Since: 1.2
 **/
void
			    ...
			    double height_in_points)
{
    ...
    _comac_pdf_surface_set_size_internal (pdf_surface,
					  width_in_points,
					  height_in_points);
    status = _comac_paginated_surface_set_size (pdf_surface->paginated_surface,
						width_in_points,
						height_in_points);
    if (status)
 ...
 *
 * The root outline item in comac_pdf_surface_add_outline().
 *
 * Since: 1.16
 **/

/**
 * comac_pdf_surface_add_outline:
 * @surface: a PDF #comac_surface_t
 * @parent_id: the id of the parent item or %COMAC_PDF_OUTLINE_ROOT if this is a top level item.
 ...
 * item with id @parent_id. Use %COMAC_PDF_OUTLINE_ROOT as the parent
			       ...
			       comac_pdf_outline_flags_t flags)
{
    ...
    status = _comac_pdf_interchange_add_outline (pdf_surface,
						 parent_id,
						 utf8,
						 link_attribs,
						 flags,
 ...
 * @utf8: metadata value
 *
 * Set document metadata. The %COMAC_PDF_METADATA_CREATE_DATE and
 * %COMAC_PDF_METADATA_MOD_DATE values must be in ISO-8601 format:
 ...
 * comac_pdf_surface_set_metadata (surface, COMAC_PDF_METADATA_TITLE, "My Document");
 * comac_pdf_surface_set_metadata (surface, COMAC_PDF_METADATA_CREATE_DATE, "2015-12-31T23:59+02:00");
				...
				const char *utf8)
{
    ...
    status = _comac_pdf_interchange_set_metadata (pdf_surface, metadata, utf8);
    if (status)
	status = _comac_surface_set_error (surface, status);
}

/**
 * comac_pdf_surface_set_custom_metadata:
 * @surface: a PDF #comac_surface_t
 ...
 * For example:
 * <informalexample><programlisting>
 * comac_pdf_surface_set_custom_metadata (surface, "ISBN", "978-0123456789");
 * </programlisting></informalexample>
 *
 * Since: 1.18
 **/
...
/**
 * comac_pdf_surface_set_page_label:
 * @surface: a PDF #comac_surface_t
 * @utf8: The page label.
 *
...
/**
 * comac_pdf_surface_set_thumbnail_size:
 * @surface: a PDF #comac_surface_t
...
_comac_pdf_surface_clear (comac_pdf_surface_t *surface)
{
    ...
    size = _comac_array_num_elements (&surface->smask_groups);
    for (i = 0; i < size; i++) {
	_comac_array_copy_element (&surface->smask_groups, i, &group);
	_comac_pdf_smask_group_destroy (group);
    }
    _comac_array_truncate (&surface->smask_groups, 0);
    _comac_array_truncate (&surface->knockout_group, 0);
    _comac_array_truncate (&surface->page_annots, 0);

    if (surface->thumbnail_image)
	comac_surface_destroy (&surface->thumbnail_image->base);
    surface->thumbnail_image = NULL;
}
...
_comac_pdf_group_resources_init (comac_pdf_group_resources_t *res)
{
    ...
    _comac_array_init (&res->alphas, sizeof (double));
    _comac_array_init (&res->smasks, sizeof (comac_pdf_resource_t));
    _comac_array_init (&res->patterns, sizeof (comac_pdf_resource_t));
    _comac_array_init (&res->shadings, sizeof (comac_pdf_resource_t));
    _comac_array_init (&res->xobjects, sizeof (comac_pdf_resource_t));
    _comac_array_init (&res->fonts, sizeof (comac_pdf_font_t));
}

static void
_comac_pdf_group_resources_fini (comac_pdf_group_resources_t *res)
{
    _comac_array_fini (&res->alphas);
    _comac_array_fini (&res->smasks);
    _comac_array_fini (&res->patterns);
    _comac_array_fini (&res->shadings);
    _comac_array_fini (&res->xobjects);
    _comac_array_fini (&res->fonts);
}

static void
_comac_pdf_group_resources_clear (comac_pdf_group_resources_t *res)
{
    int i;
    ...
    _comac_array_truncate (&res->alphas, 0);
    _comac_array_truncate (&res->smasks, 0);
    _comac_array_truncate (&res->patterns, 0);
    _comac_array_truncate (&res->shadings, 0);
    _comac_array_truncate (&res->xobjects, 0);
    _comac_array_truncate (&res->fonts, 0);
}
			      ...
			      int *index)
{
	...
	_comac_array_copy_element (&res->alphas, i, &other);
	if (alpha == other) {
	    *index = i;
	    return COMAC_STATUS_SUCCESS;
	}
    ...
    status = _comac_array_append (&res->alphas, &alpha);
    if (unlikely (status))
	return status;

    *index = _comac_array_num_elements (&res->alphas) - 1;
			     ...
			     void *closure)
{
    ...
    font.subset_resource = _comac_pdf_surface_new_object (surface);
    if (font.subset_resource.id == 0)
	return _comac_error (COMAC_STATUS_NO_MEMORY);

    status = _comac_array_append (&surface->fonts, &font);
    if (unlikely (status))
	return status;
...
static const char *
_comac_operator_to_pdf_blend_mode (comac_operator_t op)
{
    switch (op) {
					 ...
					 comac_bool_t gs0)
{
    ...
    double alpha;
    comac_pdf_resource_t *smask, *pattern, *shading, *xobject;
    comac_pdf_font_t *font;
				       ...
				       const comac_rectangle_int_t *extents)
{
    ...
    group->group_res = _comac_pdf_surface_new_object (surface);
...
_comac_pdf_smask_group_destroy (comac_pdf_smask_group_t *group)
{
    ...
    if (group->mask)
	comac_pattern_destroy (group->mask);
    free (group->utf8);
    free (group->glyphs);
    free (group->clusters);
    if (group->scaled_font)
	comac_scaled_font_destroy (group->scaled_font);
    free (group);
}
...
_comac_pdf_source_surface_equal (const void *key_a, const void *key_b)
{
    const comac_pdf_source_surface_entry_t *a = key_a;
    const comac_pdf_source_surface_entry_t *b = key_b;
...
static void
_comac_pdf_source_surface_init_key (comac_pdf_source_surface_entry_t *key)
{
    ...
    void *image_extra)
{
    ...
    case COMAC_PATTERN_TYPE_RASTER_SOURCE:
	_comac_raster_source_pattern_release (pattern, &image->base);
	break;
			     ...
			     comac_bool_t *subsurface)
{
    ...
    *bounded = TRUE;
    *subsurface = FALSE;
    if (source->type == COMAC_SURFACE_TYPE_RECORDING) {
	comac_surface_t *free_me = NULL;
 ...
 * @source_surface: [in] A #comac_surface_t to use as the source surface
 * @source_pattern: [in] A #comac_pattern_t of type SURFACE or RASTER_SOURCE to use as the source
 ...
 * surfaces in the PDF file (keyed by COMAC_MIME_TYPE_UNIQUE_ID or
    ...
    comac_rectangle_int_t *source_extents)
{
    ...
    void *image_extra;
    comac_box_t box;
    comac_rectangle_int_t op_extents;
    double x, y;
    comac_bool_t subsurface;
	...
	*source_extents = op_extents;
	...
	if (source_pattern && source_pattern->extend != COMAC_EXTEND_NONE)
	    _comac_unbounded_rectangle_init (&op_extents);

	_comac_rectangle_intersect (&op_extents, &surface_entry->extents);
	_comac_rectangle_union (&surface_entry->required_extents, &op_extents);
	...
	unique_id = _comac_malloc (surface_key.unique_id_length);
	if (unique_id == NULL) {
	    status = _comac_error (COMAC_STATUS_NO_MEMORY);
	    goto fail1;
	}
    ...
    surface_entry = _comac_malloc (sizeof (comac_pdf_source_surface_entry_t));
	...
	*y_offset = -surface_entry->extents.y;
	...
	src_surface.type = COMAC_PATTERN_TYPE_SURFACE;
	src_surface.surface = comac_surface_reference (source_surface);
	src_surface.raster_pattern = NULL;
    }

    surface_entry->surface_res = _comac_pdf_surface_new_object (surface);
	...
	status = _comac_array_append (&surface->page_surfaces, &src_surface);
	if (unlikely (status))
	    goto fail3;
    } else {
	status = _comac_array_append (&surface->doc_surfaces, &src_surface);
	if (unlikely (status))
	    goto fail3;
    }
    ...
    comac_pdf_resource_t *gstate_res)
{
    ...
    status = _comac_pattern_create_copy (&pdf_pattern.pattern, pattern);
    if (unlikely (status))
	return status;

    pdf_pattern.pattern_res = _comac_pdf_surface_new_object (surface);
...
{
    va_list ap;
    comac_pdf_resource_t self, length;
    comac_output_stream_t *output = NULL;

    if (resource) {
	self = *resource;
	_comac_pdf_surface_update_object (surface, self);
    } else {
	self = _comac_pdf_surface_new_object (surface);
	if (self.id == 0)
	    return _comac_error (COMAC_STATUS_NO_MEMORY);
    }

    length = _comac_pdf_surface_new_object (surface);
    if (length.id == 0)
	return _comac_error (COMAC_STATUS_NO_MEMORY);

    if (compressed) {
	output = _comac_deflate_stream_create (surface->output);
	if (_comac_output_stream_get_status (output))
	    return _comac_output_stream_destroy (output);
    }
    ...
    if (fmt != NULL) {
	va_start (ap, fmt);
	_comac_output_stream_vprintf (surface->output, fmt, ap);
	va_end (ap);
    }
...
_comac_pdf_surface_close_stream (comac_pdf_surface_t *surface)
{
	...
	comac_int_status_t status2;

	status2 = _comac_output_stream_destroy (surface->output);
					...
					const comac_box_double_t *bbox)
{
    ...
    _comac_output_stream_printf (surface->output,
				 ...
				 "   /Length %d\n",
				 resource.id,
				 _comac_memory_stream_length (mem_stream));
    ...
    _comac_output_stream_printf (surface->output,
				 "   >>\n"
				 "   /Resources\n");
    _comac_pdf_surface_emit_group_resources (surface, resources, TRUE);
    _comac_output_stream_printf (surface->output,
				 ">>\n"
				 "stream\n");
			       ...
			       comac_pdf_resource_t *resource)
{
    ...
    surface->group_stream.mem_stream = _comac_memory_stream_create ();

    if (surface->compress_streams) {
	surface->group_stream.stream =
	    _comac_deflate_stream_create (surface->group_stream.mem_stream);
    } else {
	surface->group_stream.stream = surface->group_stream.mem_stream;
    }
    status = _comac_output_stream_get_status (surface->group_stream.stream);
    ...
    /* Reset gstate */
    _comac_output_stream_printf (surface->output, "/gs0 gs\n");
    surface->current_pattern_is_solid_color = FALSE;
    surface->current_operator = COMAC_OPERATOR_OVER;
    _comac_pdf_operators_reset (&surface->pdf_operators);
					...
					const comac_box_double_t *bbox)
{
    ...
    status = _comac_pdf_surface_open_group (surface, bbox, NULL);
    if (unlikely (status))
	return status;
...
static comac_int_status_t
_comac_pdf_surface_close_group (comac_pdf_surface_t *surface,
				comac_pdf_resource_t *group)
{
    comac_int_status_t status = COMAC_INT_STATUS_SUCCESS, status2;
    ...
    _comac_pdf_operators_set_stream (&surface->pdf_operators, surface->output);
    surface->group_stream.active = FALSE;
    _comac_pdf_surface_write_memory_stream (surface,
...
static comac_int_status_t
_comac_pdf_surface_open_object_stream (comac_pdf_surface_t *surface)
{
	...
	_comac_array_truncate (&surface->object_stream.objects, 0);
	surface->object_stream.stream = _comac_memory_stream_create ();
...
_comac_pdf_surface_object_begin (comac_pdf_surface_t *surface,
				 comac_pdf_resource_t resource)
{
    comac_xref_stream_object_t xref_obj;
    comac_pdf_object_t *object;
    comac_int_status_t status;
...
static comac_int_status_t
_comac_pdf_surface_close_object_stream (comac_pdf_surface_t *surface)
{
    int i, num_objects;
    comac_xref_stream_object_t *xref_obj;
    long long start_pos, length;
    comac_output_stream_t *index_stream;
    comac_output_stream_t *deflate_stream;
    comac_pdf_resource_t length_res;
    comac_int_status_t status;
    comac_pdf_object_t *object;
    ...
    length_res = _comac_pdf_surface_new_object (surface);
    if (length_res.id == 0)
	return _comac_error (COMAC_STATUS_NO_MEMORY);

    _comac_pdf_surface_update_object (surface, surface->object_stream.resource);
    _comac_output_stream_printf (surface->output,
				 "%d 0 obj\n"
				 "<< /Type /ObjStm\n"
				 "   /Length %d 0 R\n"
				 ...
				 length_res.id,
				 num_objects,
				 _comac_memory_stream_length (index_stream));
    ...
    start_pos = _comac_output_stream_get_position (surface->output);
    if (surface->compress_streams) {
    ...
    status = _comac_output_stream_destroy (index_stream);
    if (unlikely (status))
	return status;

    status = _comac_output_stream_destroy (surface->object_stream.stream);
    if (unlikely (status))
	return status;
					...
					comac_bool_t is_group)
{
    ...
    surface->content_resources = _comac_pdf_surface_new_object (surface);
					 ...
					 comac_bool_t is_form)
{
    ...
    _comac_pdf_surface_update_object (surface, surface->content_resources);
    _comac_output_stream_printf (surface->output,
				 "%d 0 obj\n",
				 surface->content_resources.id);
...
static void
_comac_pdf_source_surface_entry_pluck (void *entry, void *closure)
{
    comac_pdf_source_surface_entry_t *surface_entry = entry;
    comac_hash_table_t *patterns = closure;

    _comac_hash_table_remove (patterns, &surface_entry->base);
...
static comac_status_t
_comac_pdf_surface_finish (void *abstract_surface)
{
    ...
    int size, i;
    comac_pdf_source_surface_t doc_surface;
    comac_pdf_jbig2_global_t *global;
    char *label;
    comac_pdf_resource_t xref_res;

    /* Some of the data may be in an inconistent state if there is an error status. */
    if (surface->base.status != COMAC_STATUS_SUCCESS)
	goto CLEANUP;

    _comac_pdf_surface_clear (surface);

    status = _comac_pdf_surface_open_object_stream (surface);
    if (unlikely (status))
	return status;

    /* Emit unbounded surfaces */
    _comac_pdf_surface_write_patterns_and_smask_groups (surface, TRUE);

    status = surface->base.status;
    if (status == COMAC_STATUS_SUCCESS)
	status = _comac_pdf_surface_emit_font_subsets (surface);

    status = _comac_pdf_surface_write_pages (surface);
    if (unlikely (status))
	return status;

    status = _comac_pdf_interchange_write_document_objects (surface);
    if (unlikely (status))
	return status;

    catalog = _comac_pdf_surface_new_object (surface);
    if (catalog.id == 0)
	return _comac_error (COMAC_STATUS_NO_MEMORY);

    status = _comac_pdf_surface_write_catalog (surface, catalog);
    if (unlikely (status))
	return status;

    status = _comac_pdf_surface_close_object_stream (surface);
    if (unlikely (status))
	return status;
	...
	offset = _comac_pdf_surface_write_xref (surface);
	_comac_output_stream_printf (surface->output,
				     "trailer\n"
				     "<< /Size %d\n"
				     "   /Root %d 0 R\n"
...
CLEANUP:
    ...
    status2 = _comac_pdf_operators_fini (&surface->pdf_operators);
    if (status == COMAC_STATUS_SUCCESS)
	status = status2;

    /* close any active streams still open due to fatal errors */
    status2 = _comac_pdf_surface_close_stream (surface);
    if (status == COMAC_STATUS_SUCCESS)
	status = status2;

    if (surface->group_stream.stream != NULL) {
	status2 = _comac_output_stream_destroy (surface->group_stream.stream);
	if (status == COMAC_STATUS_SUCCESS)
	    status = status2;
    }
    ...
    _comac_pdf_group_resources_fini (&surface->resources);

    _comac_array_fini (&surface->objects);
    _comac_array_fini (&surface->pages);
    _comac_array_fini (&surface->rgb_linear_functions);
    _comac_array_fini (&surface->alpha_linear_functions);
    _comac_array_fini (&surface->page_patterns);
    _comac_array_fini (&surface->page_surfaces);
    _comac_array_fini (&surface->object_stream.objects);

    size = _comac_array_num_elements (&surface->doc_surfaces);
    for (i = 0; i < size; i++) {
	_comac_array_copy_element (&surface->doc_surfaces, i, &doc_surface);
	comac_surface_destroy (doc_surface.surface);
    }
    _comac_array_fini (&surface->doc_surfaces);
    _comac_hash_table_foreach (surface->all_surfaces,
			       _comac_pdf_source_surface_entry_pluck,
			       surface->all_surfaces);
    _comac_hash_table_destroy (surface->all_surfaces);
    _comac_array_fini (&surface->smask_groups);
    _comac_array_fini (&surface->fonts);
    _comac_array_fini (&surface->knockout_group);
    _comac_array_fini (&surface->page_annots);
    _comac_array_fini (&surface->forward_links);
    ...
    _comac_array_fini (&surface->page_heights);

    size = _comac_array_num_elements (&surface->page_labels);
    for (i = 0; i < size; i++) {
	_comac_array_copy_element (&surface->page_labels, i, &label);
	free (label);
    }
    _comac_array_fini (&surface->page_labels);

    _comac_surface_clipper_reset (&surface->clipper);

    _comac_pdf_interchange_fini (surface);

    return status;
}

static comac_int_status_t
_comac_pdf_surface_start_page (void *abstract_surface)
{
    comac_pdf_surface_t *surface = abstract_surface;
    comac_pdf_resource_t page;
    comac_int_status_t status;
	...
	case COMAC_PDF_VERSION_1_4:
	    version = "1.4";
	    break;
	case COMAC_PDF_VERSION_1_5:
	    version = "1.5";
	    break;
	case COMAC_PDF_VERSION_1_6:
	    version = "1.6";
	    break;
	default:
	case COMAC_PDF_VERSION_1_7:
	    version = "1.7";
	    break;
	}
    ...
    page = _comac_pdf_surface_new_object (surface);
    if (page.id == 0)
	return _comac_error (COMAC_STATUS_NO_MEMORY);

    status = _comac_array_append (&surface->pages, &page);
    if (unlikely (status))
	return status;
...
static comac_bool_t
_comac_pdf_surface_supports_fine_grained_fallbacks (void *abstract_surface)
{
    return TRUE;
}
    ...
    comac_rectangle_int_t *source_extents)
{
    ...
    _comac_box_from_rectangle (&box, extents);
    _comac_matrix_transform_bounding_box_fixed (&source->matrix, &box, NULL);
	...
	_comac_pattern_init_for_surface (&pad_pattern, &image->base);
			       ...
			       comac_pdf_resource_t *stream_res)
{
	...
	if (transparency == COMAC_IMAGE_IS_OPAQUE) {
		...
		if (transparency == COMAC_IMAGE_HAS_ALPHA) {
		    alpha[i++] = a;
		} else { /* transparency == COMAC_IMAGE_HAS_BILEVEL_ALPHA or COMAC_IMAGE_IS_OPAQUE */
		    if (bit == 7)
			alpha[i] = 0;
		    if (a != 0)
			alpha[i] |= (1 << bit);
			       ...
			       comac_pdf_source_surface_entry_t *surface_entry)
{
    ...
    uint32_t *pixel;
    int i, x, y, bit;
    comac_pdf_resource_t smask = {0}; /* squelch bogus compiler warning */
    comac_bool_t need_smask;
	...
	if (image->format == COMAC_FORMAT_ARGB32 ||
	    ...
	    if (transparency != COMAC_IMAGE_IS_OPAQUE) {
		need_smask = TRUE;
		smask = _comac_pdf_surface_new_object (surface);
		if (smask.id == 0) {
		    status = _comac_error (COMAC_STATUS_NO_MEMORY);
		    goto CLEANUP_RGB;
		}
...
#undef IMAGE_DICTIONARY
    ...
    _comac_output_stream_write (surface->output, data, data_size);
    status = _comac_pdf_surface_close_stream (surface);
					...
					comac_pdf_jbig2_global_t **entry)
{
	...
	*entry = (comac_pdf_jbig2_global_t *) _comac_array_index (
    ...
    global.id = _comac_malloc (global_id_length);
    ...
    comac_bool_t test)
{
    ...
    comac_image_info_t info;
    const unsigned char *global_id;
    unsigned long global_id_length;
    const unsigned char *global_data;
    unsigned long global_data_length;
    comac_pdf_jbig2_global_t *global_entry = NULL; /* hide compiler warning */
    char smask_buf[30];
    ...
    comac_bool_t test)
{
    ...
    *p = 0;
    ...
    _comac_output_stream_write (surface->output, ccitt_data, ccitt_data_len);
    status = _comac_pdf_surface_close_stream (surface);
    ...
    comac_pdf_surface_t *surface, comac_pdf_source_surface_t *pdf_source)
{
    ...
    comac_paginated_mode_t old_paginated_mode;
    comac_surface_clipper_t old_clipper;
    comac_bool_t old_in_xobject;
    comac_box_double_t bbox;
    comac_int_status_t status;
    int alpha = 0;
    comac_surface_t *free_me = NULL;
    comac_surface_t *source;
    const comac_rectangle_int_t *extents;
    comac_bool_t is_subsurface;
    comac_bool_t transparency_group;
    comac_recording_surface_t *recording;

    assert (pdf_source->type == COMAC_PATTERN_TYPE_SURFACE);
    ...
    assert (source->type == COMAC_SURFACE_TYPE_RECORDING);
    recording = (comac_recording_surface_t *) source;
     ...
     */
    surface->paginated_mode = COMAC_PAGINATED_MODE_RENDER;
    _comac_pdf_group_resources_clear (&surface->resources);
    ...
    if (source->content == COMAC_CONTENT_COLOR) {
	status = _comac_pdf_surface_add_alpha (surface, 1.0, &alpha);
	if (unlikely (status))
	    goto err;
    ...
    status = _comac_pdf_surface_close_content_stream (surface, TRUE);
...
/**
 * _comac_pdf_surface_emit_surface:
 * @surface: [in] the pdf surface
 * @source: [in] #comac_pdf_source_surface_t containing the surface to write
				 ...
				 comac_bool_t *is_image)
{
	...
	status = _comac_pdf_surface_emit_jbig2_image (surface,
						      source->surface,
						      source->hash_entry,
						      test);
	...
	status = _comac_pdf_surface_emit_jpx_image (surface,
						    source->surface,
						    source->hash_entry,
						    test);
	if (status != COMAC_INT_STATUS_UNSUPPORTED) {
	...
	status = _comac_pdf_surface_emit_jpeg_image (surface,
						     source->surface,
						     source->hash_entry,
						     test);
	if (status != COMAC_INT_STATUS_UNSUPPORTED) {
	...
	status = _comac_pdf_surface_emit_ccitt_image (surface,
						      source->surface,
						      source->hash_entry,
						      test);
	if (status != COMAC_INT_STATUS_UNSUPPORTED) {
	...
	if (source->surface->type == COMAC_SURFACE_TYPE_RECORDING) {
	    if (test) {
		*is_image = FALSE;
		return COMAC_INT_STATUS_SUCCESS;
	    } else {
					 ...
					 comac_pdf_pattern_t *pdf_pattern)
{
    ...
    comac_extend_t extend = comac_pattern_get_extend (pattern);
    double xstep, ystep;
    comac_rectangle_int_t pattern_extents;
    double x_offset;
    double y_offset;
     ...
     * the pattern's matrix maps from comac's device space to comac's
     * pattern space, (both with their origin at the upper-left, and
     * comac's pattern space of size width,height).
     *
     ...
     * the PDF origin to comac's origin). We then multiply in the
     * inverse of the comac pattern matrix, (since it maps from device
     ...
     * pattern cell.
     */
    comac_p2d = pattern->matrix;
    status = comac_matrix_invert (&comac_p2d);
    ...
    comac_matrix_multiply (&pdf_p2d, &comac_p2d, &mat);
    comac_matrix_translate (&pdf_p2d, x_offset, y_offset);
    if (pdf_source->emit_image) {
	comac_matrix_translate (&pdf_p2d, 0.0, pdf_source->extents.height);
	comac_matrix_scale (&pdf_p2d, 1.0, -1.0);
    }
...
typedef struct _comac_pdf_color_stop {
    double offset;
    double color[4];
    comac_pdf_resource_t resource;
					      ...
					      comac_pdf_resource_t *function)
{
	...
	*function = elem.resource;
    ...
    status = _comac_array_append (&surface->alpha_linear_functions, &elem);
    *function = res;
				       ...
				       comac_pdf_resource_t *alpha_function)
{
	 ...
	 * is optimised into a clear pattern in comac-gstate, and
	 ...
	 */
	comac_pdf_color_stop_t pad_stops[4];

	assert (pattern->base.extend == COMAC_EXTEND_PAD);
					   ...
					   comac_pdf_resource_t gradient_mask)
{
	...
	/* When emitting a shading operator we are in comac pattern
	 * coordinates. _comac_pdf_surface_paint_gradient has set the
	 ...
	 * in the XObject below contain the comac pattern to pdf page
    ...
    smask_resource = _comac_pdf_surface_new_object (surface);
				    ...
				    comac_pdf_resource_t color_function)
{
    ...
    if (pdf_pattern->pattern->type == COMAC_PATTERN_TYPE_LINEAR) {
	_comac_output_stream_printf (surface->output,
				     "      << /ShadingType 2\n"
				     "         /ColorSpace %s\n"
				     "         /Coords [ %f %f %f %f ]\n",
				  ...
				  comac_pdf_pattern_t *pdf_pattern)
{
    ...
    status = comac_matrix_invert (&pat_to_pdf);
    /* comac_pattern_set_matrix ensures the matrix is invertible */
    assert (status == COMAC_INT_STATUS_SUCCESS);
	 ...
	 * _comac_ps_surface_emit_pattern_stops emits it as a stitched
    ...
    _comac_gradient_pattern_interpolate (pattern, domain[0], &start);
				 ...
				 comac_pdf_pattern_t *pdf_pattern)
{
    ...
    case COMAC_PATTERN_TYPE_SOLID:
	ASSERT_NOT_REACHED;
	status = _comac_error (COMAC_STATUS_PATTERN_TYPE_MISMATCH);
	break;

    case COMAC_PATTERN_TYPE_SURFACE:
    case COMAC_PATTERN_TYPE_RASTER_SOURCE:
	status = _comac_pdf_surface_emit_surface_pattern (surface, pdf_pattern);
	break;

    case COMAC_PATTERN_TYPE_LINEAR:
    case COMAC_PATTERN_TYPE_RADIAL:
	status = _comac_pdf_surface_emit_gradient (surface, pdf_pattern);
	break;

    case COMAC_PATTERN_TYPE_MESH:
	status = _comac_pdf_surface_emit_mesh_pattern (surface, pdf_pattern);
	break;
					  ...
					  comac_bool_t stencil_mask)
{
    ...
    comac_p2d = source->matrix;
    status = comac_matrix_invert (&comac_p2d);
    /* comac_pattern_set_matrix ensures the matrix is invertible */
    assert (status == COMAC_INT_STATUS_SUCCESS);

    pdf_p2d = surface->comac_to_pdf;
    comac_matrix_multiply (&pdf_p2d, &comac_p2d, &pdf_p2d);
    comac_matrix_translate (&pdf_p2d, x_offset, y_offset);
    if (pdf_source->emit_image) {
	int width, height;
	...
	comac_matrix_translate (&pdf_p2d, 0.0, height);
	comac_matrix_scale (&pdf_p2d, 1.0, -1.0);
	comac_matrix_scale (&pdf_p2d, width, height);
    }
    ...
    if (! _comac_matrix_is_identity (&pdf_p2d)) {
	_comac_output_stream_print_matrix (surface->output, &pdf_p2d);
	_comac_output_stream_printf (surface->output, " cm\n");
    }

    status = _comac_pdf_surface_add_alpha (surface, alpha, &alpha_id);
    if (unlikely (status))
	return status;
    ...
    return _comac_pdf_surface_add_xobject (surface, pdf_source->surface_res);
}
				   ...
				   double alpha)
{
    ...
    /* comac_pattern_set_matrix ensures the matrix is invertible */
    assert (status == COMAC_INT_STATUS_SUCCESS);
    comac_matrix_multiply (&pat_to_pdf, &pat_to_pdf, &surface->comac_to_pdf);

    status = _comac_pdf_operators_flush (&surface->pdf_operators);
    if (unlikely (status))
	return status;

    if (! _comac_matrix_is_identity (&pat_to_pdf)) {
	_comac_output_stream_print_matrix (surface->output, &pat_to_pdf);
	_comac_output_stream_printf (surface->output, " cm\n");
    }

    status = _comac_pdf_surface_add_shading (surface, shading_res);
    if (unlikely (status))
	return status;

    if (gstate_res.id != 0) {
	status = _comac_pdf_surface_add_smask (surface, gstate_res);
	if (unlikely (status))
	    return status;
				  ...
				  comac_bool_t mask)
{
    ...
    case COMAC_PATTERN_TYPE_SURFACE:
    case COMAC_PATTERN_TYPE_RASTER_SOURCE:
	return _comac_pdf_surface_paint_surface_pattern (surface,
							 op,
							 source,
							 extents,
							 alpha,
    ...
    case COMAC_PATTERN_TYPE_RADIAL:
    case COMAC_PATTERN_TYPE_MESH:
	return _comac_pdf_surface_paint_gradient (surface,
						  op,
						  source,
						  extents,
						  alpha);
...
static comac_bool_t
_can_paint_pattern (const comac_pattern_t *pattern)
{
    switch (pattern->type) {
    case COMAC_PATTERN_TYPE_SOLID:
	return FALSE;
    ...
    case COMAC_PATTERN_TYPE_SURFACE:
    case COMAC_PATTERN_TYPE_RASTER_SOURCE:
	return (pattern->extend == COMAC_EXTEND_NONE ||
		pattern->extend == COMAC_EXTEND_PAD);
				   ...
				   comac_bool_t is_stroke)
{
	...
	status = _comac_pdf_surface_add_pattern (surface, pattern_res);
	if (unlikely (status))
	    return status;
...
static comac_int_status_t
_comac_pdf_surface_unselect_pattern (comac_pdf_surface_t *surface)
{
    comac_int_status_t status;
...
static comac_int_status_t
_comac_pdf_surface_show_page (void *abstract_surface)
{
    comac_pdf_surface_t *surface = abstract_surface;
    comac_int_status_t status;

    status = _comac_array_append (&surface->page_heights, &surface->height);
    if (unlikely (status))
	return status;
    ...
    status = _comac_pdf_interchange_end_page_content (surface);
    if (unlikely (status))
	return status;

    status = _comac_pdf_surface_close_content_stream (surface, FALSE);
    if (unlikely (status))
	return status;

    _comac_surface_clipper_reset (&surface->clipper);

    status = _comac_pdf_surface_write_page (surface);
    if (unlikely (status))
	return status;
...
_comac_pdf_surface_write_pages (comac_pdf_surface_t *surface)
{
    ...
    status = _comac_pdf_surface_object_begin (surface, surface->pages_resource);
    if (unlikely (status))
	return status;
...
_comac_utf8_to_pdf_string (const char *utf8, char **str_out)
{
	...
	status = _comac_utf8_to_utf16 (utf8, -1, &utf16, &utf16_len);
	if (unlikely (status))
	    return status;
					   ...
					   const char *utf8)
{
    ...
    if (utf8 && *utf8) {
	status = _comac_utf8_to_utf16 (utf8, -1, &utf16, &utf16_len);
	if (unlikely (status == COMAC_INT_STATUS_INVALID_STRING)) {
	    utf16 = NULL;
	    utf16_len = 0;
				  ...
				  comac_cff_subset_t *subset)
{
    ...
    descriptor = _comac_pdf_surface_new_object (surface);
    if (descriptor.id == 0)
	return _comac_error (COMAC_STATUS_NO_MEMORY);
	...
	status = _comac_utf8_to_pdf_string (subset->family_name_utf8, &pdf_str);
	if (likely (status == COMAC_INT_STATUS_SUCCESS)) {
	    _comac_output_stream_printf (surface->output,
					 "   /FontFamily %s\n",
					 pdf_str);
	    free (pdf_str);
	} else if (status != COMAC_INT_STATUS_INVALID_STRING) {
	    return status;
	}
    }
	...
	cidfont_dict = _comac_pdf_surface_new_object (surface);
    ...
    comac_pdf_surface_t *surface, comac_scaled_font_subset_t *font_subset)
{
    ...
    _comac_cff_fallback_fini (&subset);
				    ...
				    comac_type1_subset_t *subset)
{
    ...
    stream = surface->pdf_stream.self;
    _comac_output_stream_write (surface->output, subset->data, length);
    status = _comac_pdf_surface_close_stream (surface);
    if (unlikely (status))
	return status;
    ...
    font.subset_id = font_subset->subset_id;
    font.subset_resource = subset_resource;
    return _comac_array_append (&surface->fonts, &font);
}
    ...
    comac_pdf_surface_t *surface, comac_scaled_font_subset_t *font_subset)
{
    ...
    status = _comac_pdf_surface_emit_type1_font (surface, font_subset, &subset);

    _comac_type1_subset_fini (&subset);
    return status;
}
    ...
    comac_pdf_surface_t *surface, comac_scaled_font_subset_t *font_subset)
{
    ...
    _comac_type1_fallback_fini (&subset);
    return status;
}
    ...
    comac_pdf_surface_t *surface, comac_scaled_font_subset_t *font_subset)
{
    ...
    comac_int_status_t status;
    comac_pdf_font_t font;
    comac_truetype_subset_t subset;
    unsigned int i, last_glyph;
    char tag[10];
    ...
    status = _comac_truetype_subset_init_pdf (&subset, font_subset);
    if (unlikely (status))
	return status;
			   ...
			   comac_output_stream_t *stream)
{
     ...
     * masks */
    assert (image->format == COMAC_FORMAT_A1);
	...
	for (col = 0; col < num_cols; col++) {
	    output_byte = COMAC_BITSWAP8_IF_LITTLE_ENDIAN (*byte);
	    _comac_output_stream_write (stream, &output_byte, 1);
	    byte++;
	}
    }
    ...
    comac_pdf_surface_t *surface, comac_scaled_font_subset_t *font_subset)
{
	...
	status = _comac_type3_glyph_surface_emit_glyph (type3_surface,
							surface->output,
							font_subset->glyphs[i],
							&bbox,
							&widths[i]);
    ...
    encoding = _comac_pdf_surface_new_object (surface);
    if (encoding.id == 0) {
	free (glyphs);
	free (widths);
	return _comac_error (COMAC_STATUS_NO_MEMORY);
    ...
    char_procs = _comac_pdf_surface_new_object (surface);
    if (char_procs.id == 0) {
	free (glyphs);
	free (widths);
	return _comac_error (COMAC_STATUS_NO_MEMORY);
    ...
    _comac_output_stream_printf (surface->output,
				 ...
				 "   /LastChar %d\n",
				 subset_resource.id,
				 _comac_fixed_to_double (font_bbox.p1.x),
				 _comac_fixed_to_double (font_bbox.p1.y),
				 _comac_fixed_to_double (font_bbox.p2.x),
				 _comac_fixed_to_double (font_bbox.p2.y),
				 encoding.id,
				 char_procs.id,
    ...
    comac_scaled_font_subset_t *font_subset, void *closure)
{
    ...
    status = _comac_pdf_surface_emit_cff_font_subset (surface, font_subset);
    if (status != COMAC_INT_STATUS_UNSUPPORTED)
	return status;
    ...
    status = _comac_pdf_surface_emit_type1_font_subset (surface, font_subset);
    if (status != COMAC_INT_STATUS_UNSUPPORTED)
	return status;

    status = _comac_pdf_surface_emit_cff_fallback_font (surface, font_subset);
    if (status != COMAC_INT_STATUS_UNSUPPORTED)
	return status;

    status = _comac_pdf_surface_emit_type1_fallback_font (surface, font_subset);
    if (status != COMAC_INT_STATUS_UNSUPPORTED)
	return status;
    ...
    comac_scaled_font_subset_t *font_subset, void *closure)
{
    ...
    status = _comac_pdf_surface_emit_type3_font_subset (surface, font_subset);
    if (status != COMAC_INT_STATUS_UNSUPPORTED)
	return status;
				  ...
				  comac_pdf_resource_t catalog)
{
    ...
    status = _comac_pdf_surface_object_begin (surface, catalog);
    if (unlikely (status))
	return status;
...
_comac_pdf_surface_write_xref (comac_pdf_surface_t *surface)
{
    ...
    num_objects = _comac_array_num_elements (&surface->objects);

    offset = _comac_output_stream_get_position (surface->output);
    _comac_output_stream_printf (surface->output,
				 "xref\n"
				 "%d %d\n",
				 0,
				...
				comac_bool_t write_as_comments)
{
	...
	_comac_output_stream_write (stream, buf, field2_size + 3);
    }
}
				      ...
				      long long *xref_offset)
{
    ...
    *xref_offset = _comac_output_stream_get_position (surface->output);
    ...
    _comac_pdf_surface_update_object (surface, xref_res);
    _comac_output_stream_printf (surface->output,
				 "%d 0 obj\n"
				 "<< /Type /XRef\n"
				 "   /Length %d\n"
				     ...
				     comac_pdf_smask_group_t *group)
{
    ...
    comac_pdf_resource_t smask;
    comac_pdf_smask_group_t *smask_group;
    comac_pdf_resource_t pattern_res, gstate_res;
    comac_int_status_t status;
    comac_box_double_t bbox;
	...
	_comac_output_stream_printf (surface->output, "q\n");
	status = _comac_pdf_surface_paint_pattern (surface,
						   COMAC_OPERATOR_OVER,
						   group->mask,
						   &group->extents,
						   1.0,	   /* alpha */
	...
	if (gstate_res.id != 0) {
	    ...
	    smask_group->source = comac_pattern_reference (group->mask);
	    smask_group->source_res = pattern_res;
	    status = _comac_pdf_surface_add_smask_group (surface, smask_group);
	    if (unlikely (status)) {
		_comac_pdf_smask_group_destroy (smask_group);
		return status;
	    }
	    ...
	    status = _comac_pdf_surface_unselect_pattern (surface);
	    if (unlikely (status))
		return status;
	}
    }

    status = _comac_pdf_surface_close_group (surface, &mask_group);
    if (unlikely (status))
	return status;

    /* Create source group */
    status = _comac_pdf_surface_open_group (surface, &bbox, &group->source_res);
    if (unlikely (status))
	return status;
	...
	if (gstate_res.id != 0) {
	    ...
	    smask_group->operation = PDF_PAINT;
	    smask_group->source = comac_pattern_reference (group->source);
	    smask_group->source_res = pattern_res;
	    status = _comac_pdf_surface_add_smask_group (surface, smask_group);
	    if (unlikely (status)) {
		_comac_pdf_smask_group_destroy (smask_group);
    ...
    status = _comac_pdf_surface_close_group (surface, NULL);
    if (unlikely (status))
	return status;
    ...
    _comac_pdf_surface_update_object (surface, group->group_res);
    _comac_output_stream_printf (surface->output,
				 "%d 0 obj\n"
				 "<< /Type /ExtGState\n"
				 "   /SMask %d 0 R\n"
				      ...
				      comac_pdf_smask_group_t *group)
{
	...
	status = _comac_pdf_surface_write_mask_group (surface, group);
	goto RESTORE_SIZE;
    }

    _get_bbox_from_extents (&group->extents, &bbox);
    status = _comac_pdf_surface_open_group (surface, &bbox, &group->group_res);
    if (unlikely (status))
	return status;
	...
	break;
    case PDF_FILL:
	status = _comac_pdf_operators_fill (&surface->pdf_operators,
					    &group->path,
					    group->fill_rule);
	break;
    case PDF_STROKE:
	status = _comac_pdf_operators_stroke (&surface->pdf_operators,
					      &group->path,
					      &group->style,
					      &group->ctm,
					      &group->ctm_inverse);
...
_comac_pdf_surface_write_page (comac_pdf_surface_t *surface)
{
    comac_pdf_resource_t knockout, res, thumbnail_res;
    comac_pdf_resource_t *page;
    comac_int_status_t status;
    ...
    status = _comac_pdf_interchange_write_page_objects (surface);
    if (unlikely (status))
	return status;
	...
	_get_bbox_from_extents (&extents, &bbox);
	status = _comac_pdf_surface_open_knockout_group (surface, &bbox);
	if (unlikely (status))
	    return status;
	...
	_comac_output_stream_printf (surface->output,
				     "/x%d Do\n",
				     surface->content.id);
	status = _comac_pdf_surface_add_xobject (surface, surface->content);
	if (unlikely (status))
	    return status;

	status = _comac_pdf_surface_close_group (surface, &knockout);
	if (unlikely (status))
	    return status;
    ...
    thumbnail_res.id = 0;
    if (surface->thumbnail_image) {
	comac_pdf_source_surface_entry_t entry;
    ...
    page = _comac_array_index (&surface->pages, page_num - 1);

    status = _comac_pdf_surface_object_begin (surface, *page);
    if (unlikely (status))
	return status;
    ...
    num_annots = _comac_array_num_elements (&surface->page_annots);
    if (num_annots > 0) {
	_comac_output_stream_printf (surface->object_stream.stream,
				     "   /Annots [ ");
	for (i = 0; i < num_annots; i++) {
	    _comac_array_copy_element (&surface->page_annots, i, &res);
	    _comac_output_stream_printf (surface->object_stream.stream,
					 "%d 0 R ",
					 res.id);
	}
    ...
    return _comac_pdf_surface_close_object_stream (surface);
}
    ...
    comac_pdf_surface_t *surface, comac_surface_pattern_t *pattern)
{
	...
	status = COMAC_INT_STATUS_FLATTEN_TRANSPARENCY;
...
static comac_bool_t
_surface_pattern_supported (comac_surface_pattern_t *pattern)
{
...
*/
    ...
    extend = comac_pattern_get_extend (&pattern->base);
    switch (extend) {
    case COMAC_EXTEND_NONE:
    case COMAC_EXTEND_REPEAT:
    case COMAC_EXTEND_REFLECT:
...
static comac_bool_t
_pattern_supported (const comac_pattern_t *pattern)
{
    switch (pattern->type) {
    case COMAC_PATTERN_TYPE_SOLID:
    case COMAC_PATTERN_TYPE_LINEAR:
    ...
    case COMAC_PATTERN_TYPE_SURFACE:
	return _surface_pattern_supported ((comac_surface_pattern_t *) pattern);
...
static comac_bool_t
_pdf_operator_supported (comac_operator_t op)
{
    switch (op) {
    case COMAC_OPERATOR_OVER:
    case COMAC_OPERATOR_MULTIPLY:
...
static comac_int_status_t
_comac_pdf_surface_start_fallback (comac_pdf_surface_t *surface)
{
    comac_box_double_t bbox;
    comac_int_status_t status;
    ...
    status = _comac_array_append (&surface->knockout_group, &surface->content);
    if (unlikely (status))
	return status;
    ...
    return _comac_pdf_interchange_begin_page_content (surface);
}
					...
					const comac_rectangle_int_t *extents)
{
    ...
    src_x2 = src_width;
    src_y2 = src_height;
    comac_matrix_transform_point (&p2u, &src_x1, &src_y1);
    comac_matrix_transform_point (&p2u, &src_x2, &src_y2);
    src_radius = _comac_matrix_transformed_circle_major_axis (&p2u, 0.5);
    ...
    mask_x2 = mask_width;
    mask_y2 = mask_height;
    comac_matrix_transform_point (&p2u, &mask_x1, &mask_y1);
    comac_matrix_transform_point (&p2u, &mask_x2, &mask_y2);
    mask_radius = _comac_matrix_transformed_circle_major_axis (&p2u, 0.5);
...
static comac_int_status_t
_comac_pdf_surface_set_clip (comac_pdf_surface_t *surface,
			     comac_composite_rectangles_t *composite)
{
    comac_clip_t *clip = composite->clip;

    if (_comac_composite_rectangles_can_reduce_clip (composite, clip))
	clip = NULL;

    if (clip == NULL) {
	if (_comac_composite_rectangles_can_reduce_clip (composite,
							 surface->clipper.clip))
	    return COMAC_STATUS_SUCCESS;
    }

    return _comac_surface_clipper_set_clip (&surface->clipper, clip);
}
			  ...
			  const comac_clip_t *clip)
{
    ...
    status = _comac_pdf_surface_select_operator (surface, op);
    if (unlikely (status))
	goto cleanup;
	...
	status = _comac_pattern_create_copy (&group->source, source);
	if (unlikely (status)) {
	    _comac_pdf_smask_group_destroy (group);
	    goto cleanup;
	}
	group->source_res = pattern_res;
	status = _comac_pdf_surface_add_smask_group (surface, group);
	if (unlikely (status)) {
	    _comac_pdf_smask_group_destroy (group);
	    goto cleanup;
	}
	...
	status = _comac_pdf_surface_add_xobject (surface, group->group_res);
	if (unlikely (status))
	    goto cleanup;
			 ...
			 const comac_clip_t *clip)
{
	...
	_comac_composite_rectangles_fini (&extents);
	return _comac_analysis_surface_merge_status (source_status,
						     mask_status);
    } else if (surface->paginated_mode == COMAC_PAGINATED_MODE_FALLBACK) {
	status = _comac_pdf_surface_start_fallback (surface);
	if (unlikely (status))
    ...
    status = _comac_pattern_get_ink_extents (mask, &r);
    if (unlikely (status))
	goto cleanup;
    ...
    if (_comac_pattern_is_constant_alpha (mask, &extents.bounded, &alpha) &&
    ...
    status = _comac_pattern_create_copy (&group->mask, mask);
    if (unlikely (status)) {
	_comac_pdf_smask_group_destroy (group);
	goto cleanup;
    }
    group->source_res = _comac_pdf_surface_new_object (surface);
    ...
    status = _comac_pdf_surface_add_smask (surface, group->group_res);
    if (unlikely (status))
	goto cleanup;

    status = _comac_pdf_surface_add_xobject (surface, group->source_res);
    if (unlikely (status))
	goto cleanup;
			   ...
			   const comac_clip_t *clip)
{
	...
	group->source_res = pattern_res;
	status = _comac_path_fixed_init_copy (&group->path, path);
	if (unlikely (status)) {
	    _comac_pdf_smask_group_destroy (group);
	    goto cleanup;
	}
				...
				const comac_clip_t *clip)
{
    comac_pdf_surface_t *surface = abstract_surface;
    comac_int_status_t status;
    comac_pdf_resource_t fill_pattern_res, stroke_pattern_res, gstate_res;
    comac_composite_rectangles_t extents;
    ...
    status = _comac_pdf_surface_select_operator (surface, fill_op);
    if (unlikely (status))
	goto cleanup;
    ...
    status = _comac_pdf_operators_fill_stroke (&surface->pdf_operators,
					       path,
					       fill_rule,
					       stroke_style,
					       stroke_ctm,
				     ...
				     const comac_clip_t *clip)
{
	...
	if (utf8_len) {
	    group->utf8 = _comac_malloc (utf8_len);
	...
	if (num_glyphs) {
	    ...
	    memcpy (group->glyphs, glyphs, sizeof (comac_glyph_t) * num_glyphs);
	}
	...
	group->scaled_font = comac_scaled_font_reference (scaled_font);
	status = _comac_pdf_surface_add_smask_group (surface, group);
	if (unlikely (status)) {
	    _comac_pdf_smask_group_destroy (group);
	    goto cleanup;
	 ...
	 * each other. */
	if (! _comac_pattern_is_opaque (source, &extents.bounded)) {
	    status = _comac_pdf_operators_flush (&surface->pdf_operators);
	    if (unlikely (status))
		goto cleanup;
	}
				       ...
				       comac_paginated_mode_t paginated_mode)
{
    ...
    status = _comac_pdf_interchange_begin_page_content (surface);
    if (unlikely (status))
	return status;
...
static const comac_surface_backend_t comac_pdf_surface_backend = {
    ...
    _comac_pdf_surface_get_font_options,

    NULL, /* flush */
    NULL, /* mark_dirty_rectangle */

    /* Here are the drawing functions */
    _comac_pdf_surface_paint,
    _comac_pdf_surface_mask,
    _comac_pdf_surface_stroke,
    _comac_pdf_surface_fill,
    _comac_pdf_surface_fill_stroke,
    NULL, /* show_glyphs */
    _comac_pdf_surface_has_show_text_glyphs,
    _comac_pdf_surface_show_text_glyphs,
    _comac_pdf_surface_get_supported_mime_types,
    _comac_pdf_surface_tag,
};
