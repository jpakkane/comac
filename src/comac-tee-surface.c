			 ...
			 const comac_clip_t	*clip)
{
    ...
    slaves = _comac_array_index (&surface->slaves, 0);
    for (n = 0; n < num_slaves; n++) {
	status = _comac_surface_wrapper_mask (&slaves[n],
					      op, source, mask, clip);
	if (unlikely (status))
	    return status;
    }
				     ...
				     const comac_clip_t	    *clip)
{
	...
	status = _comac_surface_wrapper_show_text_glyphs (&slaves[n], op,
							  source,
							  utf8, utf8_len,
    ...
    memcpy (glyphs_copy, glyphs, sizeof (comac_glyph_t) * num_glyphs);
    status = _comac_surface_wrapper_show_text_glyphs (&surface->master, op,
						      source,
						      utf8, utf8_len,
						      glyphs_copy, num_glyphs,
						      clusters, num_clusters,
		       ...
		       comac_surface_t *target)
{
	...
	status = _comac_surface_set_error (abstract_surface,
...
comac_surface_t *
comac_tee_surface_index (comac_surface_t *abstract_surface,
			 unsigned int index)
{
    comac_tee_surface_t *surface;
    ...
    if (abstract_surface->backend != &comac_tee_surface_backend)
	return _comac_surface_create_in_error (_comac_error (COMAC_STATUS_SURFACE_TYPE_MISMATCH));
	...
	if (index >= _comac_array_num_elements (&surface->slaves))
	    return _comac_surface_create_in_error (_comac_error (COMAC_STATUS_INVALID_INDEX));
