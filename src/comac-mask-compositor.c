...
(*draw_func_t) (const comac_mask_compositor_t *compositor,
		comac_surface_t			*dst,
		void				*closure,
		comac_operator_t		 op,
		   ...
		   uint16_t coverage)
{
    ...
    info->compositor->fill_rectangles (info->dst, COMAC_OPERATOR_IN,
				       &color, &rect, 1);
}
		       ...
		       const comac_composite_rectangles_t *extents)
{
	...
	status = compositor->fill_rectangles (surface, COMAC_OPERATOR_CLEAR,
	...
	status = mask_func (compositor, surface, draw_closure,
			    COMAC_OPERATOR_SOURCE, NULL, NULL,
			    ...
			    const comac_composite_rectangles_t*extents)
{
    ...
    compositor->composite (tmp, COMAC_OPERATOR_SOURCE, dst, NULL,
			   extents->bounded.x,      extents->bounded.y,
			   0, 0,
			   0, 0,
			   extents->bounded.width,  extents->bounded.height);
	...
	compositor->composite (dst, COMAC_OPERATOR_DEST_OUT, clip, NULL,
			       clip_x, clip_y,
			       0, 0,
			       extents->bounded.x,     extents->bounded.y,
			       extents->bounded.width, extents->bounded.height);
	...
	compositor->composite (dst, COMAC_OPERATOR_ADD, tmp, clip,
			       0, 0,
			       clip_x, clip_y,
			       extents->bounded.x,     extents->bounded.y,
			       extents->bounded.width, extents->bounded.height);
			   ...
			   const comac_composite_rectangles_t	*extents)
{
	...
	compositor->composite (dst, COMAC_OPERATOR_ADD, src, mask,
			       extents->bounded.x + src_x, extents->bounded.y + src_y,
			       0, 0,
			       extents->bounded.x,     extents->bounded.y,
			       extents->bounded.width, extents->bounded.height);
		 ...
		 const comac_composite_rectangles_t *extents)
{
    ...
    return compositor->fill_rectangles (dst, COMAC_OPERATOR_CLEAR,
					COMAC_COLOR_TRANSPARENT,
					rects, n);
}
		       ...
		       comac_boxes_t *boxes)
{
	...
	comac_region_contains_rectangle (clip_region,
					 &extents->bounded) == COMAC_REGION_OVERLAP_IN)
	clip_region = NULL;
	...
	status = _comac_bentley_ottmann_tessellate_boxes (&clear,
							  COMAC_FILL_RULE_WINDING,
							  &clear);
    }
	      ...
	      comac_boxes_t *boxes)
{
    ...
    src = _comac_pattern_get_source ((comac_surface_pattern_t *)source, &limit);
...
_comac_mask_compositor_paint (const comac_compositor_t *_compositor,
			      comac_composite_rectangles_t *extents)
{
    comac_mask_compositor_t *compositor = (comac_mask_compositor_t*)_compositor;
    comac_boxes_t boxes;
    comac_int_status_t status;
...
_comac_mask_compositor_mask (const comac_compositor_t *_compositor,
			     comac_composite_rectangles_t *extents)
{
    const comac_mask_compositor_t *compositor = (comac_mask_compositor_t*)_compositor;
	...
	extents->clip->path == NULL &&
	_comac_clip_is_region (extents->clip)) {
