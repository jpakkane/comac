...
#ifndef COMAC_PAGINATED_H
#define COMAC_PAGINATED_H

#include "comacint.h"

struct _comac_paginated_surface_backend {
 ...
 * 'real' surface using _comac_surface_init() and the standard
 * #comac_surface_backend_t. Then you also call
 * _comac_paginated_surface_create which takes its own, much simpler,
 * #comac_paginated_surface_backend_t. You are free to return the result
 * of _comac_paginated_surface_create() from your public
 * comac_<foo>_surface_create(). The paginated backend will be careful
 ...
 * comac_show_page(), the paginated surface performs the following
 * sequence of operations (using the backend functions passed to
 * comac_paginated_surface_create()):
 *
 ...
 *    its output.
 *
 * 2. Calls set_paginated_mode() with an argument of %COMAC_PAGINATED_MODE_ANALYZE
 *
 ...
 *
 * 5. Calls set_paginated_mode() with an argument of %COMAC_PAGINATED_MODE_RENDER
 *
 * 6. Replays a subset of the recording-surface operations to the target surface
 *
 * 7. Calls set_paginated_mode() with an argument of %COMAC_PAGINATED_MODE_FALLBACK
 *
 ...
 * return %COMAC_STATUS_SUCCESS or %COMAC_INT_STATUS_UNSUPPORTED to
