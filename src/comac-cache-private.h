...
#ifndef COMAC_CACHE_PRIVATE_H
#define COMAC_CACHE_PRIVATE_H
 ...
 * _comac_cache_entry:
 *
 * A #comac_cache_entry_t contains both a key and a value for
 * #comac_cache_t. User-derived types for #comac_cache_entry_t must
 * have a #comac_cache_entry_t as their first field. For example:
 *
 * 	typedef _my_entry {
 *	    comac_cache_entry_t base;
 *	    ... Remainder of key and value fields here ..
 *	} my_entry_t;
 *
 * which then allows a pointer to my_entry_t to be passed to any of
 * the #comac_cache_t functions as follows without requiring a cast:
 *
 *	_comac_cache_insert (cache, &my_entry->base, size);
 *
 ...
 * the max_size parameter passed to _comac_cache_create()). If all
 ...
 * function). A few of the #comac_cache_t functions accept an entry which
 ...
 **/
typedef struct _comac_cache_entry {
    uintptr_t hash;
    unsigned long size;
} comac_cache_entry_t;

typedef comac_bool_t (*comac_cache_predicate_func_t) (const void *entry);
