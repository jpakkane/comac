...
} comac_charstring_type_t;

typedef struct _comac_type1_font {
    int *widths;
                         ...
                         comac_bool_t                 hex_encode)
{
    ...
    font = calloc (1, sizeof (comac_type1_font_t));
 ...
 */
static void
charstring_encode_command (comac_array_t *data, int command)
{
    comac_status_t status;
    ...
    *p++ = command & 0x00ff;
    ...
    orig_size = _comac_array_size (data);
    status = _comac_array_append_multiple (data, buf, p - buf);

    assert (status == COMAC_STATUS_SUCCESS);
    assert (_comac_array_size (data) == orig_size);
}
...
static void
charstring_encrypt (comac_array_t *data)
{
    unsigned char *d, *end;
    uint16_t c, p, r;

    r = COMAC_TYPE1_CHARSTRING_KEY;
    d = (unsigned char *) _comac_array_index (data, 0);
    end = d + _comac_array_num_elements (data);
    while (d < end) {
	p = *d;
	c = p ^ (r >> 8);
                                    ...
                                    comac_array_t           *data)
{
    ...
    if (glyph_index == 0 && status == COMAC_INT_STATUS_UNSUPPORTED) {
	emit_path = FALSE;
	status = _comac_scaled_glyph_lookup (font->type1_scaled_font,
					     glyph_index,
					     COMAC_SCALED_GLYPH_INFO_METRICS,
    ...
    path_info.type = type;
    if (emit_path) {
	status = _comac_path_fixed_interpret (scaled_glyph->path,
					      _charstring_move_to,
					      _charstring_line_to,
                               ...
                               const char         *name)
{
    ...
    _comac_output_stream_printf (font->output, "/FontBBox {");
    font->bbox_position = _comac_output_stream_get_position (font->output);
                        ...
                        const char *name)
{
    ...
    comac_type1_font_write_header (font, name);
    font->header_size = _comac_output_stream_get_position (font->output);

    status = comac_type1_font_write_private_dict (font, name);
    if (unlikely (status))
	return status;
...
static comac_int_status_t
comac_type1_font_generate (comac_type1_font_t *font, const char *name)
{
    comac_int_status_t status;
    ...
    status = comac_type1_font_write (font, name);
    if (unlikely (status))
	return status;

    font->data = _comac_array_index (&font->contents, 0);

    return COMAC_STATUS_SUCCESS;
}

static comac_status_t
comac_type1_font_destroy (comac_type1_font_t *font)
{
    comac_status_t status = COMAC_STATUS_SUCCESS;

    free (font->widths);
    comac_scaled_font_destroy (font->type1_scaled_font);
                                     ...
                                     comac_bool_t                hex_encode)
{
    ...
    status = comac_type1_font_create (scaled_font_subset, &font, hex_encode);
    if (unlikely (status))
	return status;

    status = comac_type1_font_generate (font, name);
    if (unlikely (status))
	goto fail1;
                               ...
                               comac_scaled_font_subset_t *scaled_font_subset)
{
    ...
    status = comac_type1_font_create (scaled_font_subset, &font, FALSE);
    if (unlikely (status))
	return status;
...
void
_comac_type2_charstrings_fini (comac_type2_charstrings_t *type2_subset)
{
